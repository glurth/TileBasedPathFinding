...
namespace EyE.Maps.Templates
{
    ...
    public class MazeMapRadial : MazeMap2D<RadialCoord>//GenericMazeMap<RadialCoord>
    {
        ...
        public MazeMapRadial(int rings, int baseSectors, Vector3 mazeNormal, int numTeleportTiles = 0, bool alwaysReverseTeleport = true, float worldScale = 1f, int numSolutions = 1, int oneWayTileCount = 0)
            : base(new RadialCoord(rings, baseSectors), new RadialCoord(0, 0), new RadialCoord(rings - 1, 0), mazeNormal,  numTeleportTiles,  alwaysReverseTeleport, worldScale, numSolutions, oneWayTileCount)
        {
            this.rings = Mathf.Max(1, rings);
            this.baseSectors = Mathf.Max(1, baseSectors);
            RadialCoord.mapRef = this;
        }
        public MazeMapRadial(int rings, int baseSectors = 6, int numTeleportTiles = 0, bool alwaysReverseTeleport = true, float worldScale = 1f, int numSolutions = 1, int oneWayTileCount = 0)
            : base(new RadialCoord(rings, baseSectors), new RadialCoord(0, 0), new RadialCoord(rings - 1, 0), numTeleportTiles, alwaysReverseTeleport, worldScale, numSolutions,  oneWayTileCount)
        {
