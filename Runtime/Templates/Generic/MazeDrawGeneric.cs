...
public abstract class MazeDrawGeneric<T> : MonoBehaviour where T : ITileCoordinate<T>
{
    ...
    public GameObject wallPrefab;              // Prefab for walls
    public GameObject floorPrefab;             // Prefab for floor tiles
    public bool drawBorderWalls = true;        // New member to determine if border walls should be drawn
    ...
    private void GenerateMazeVisuals()
    {
        float wallThickness = tileScale * wallThicknessFraction;
        if (floorPrefab != null)
        {
            foreach (T coord in maze.allMapCoords)
            {
                Vector3 tilePosition = maze.GetWorldPosition(coord);
                GameObject floor = Instantiate(floorPrefab, tilePosition, Quaternion.identity, transform);
                instantiatedObjectsList.Add(floor);
            }
        }
        foreach (T coord in maze.allMapCoords)
        {
            ...
            for (int i = 0; i < neighborCount; i++)
            {
                if (wallsForTile[i])
                {
                    ...
                    wall.name = $"Wall[{coord}][{i}]";
                    instantiatedObjectsList.Add(wall);
    ...
    {
        foreach (GameObject obj in instantiatedObjectsList)
        {
            Destroy(obj);
        }
