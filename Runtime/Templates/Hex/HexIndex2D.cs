...
/// </summary>
    ...
    public struct TriangularIndex2D : ITileCoordinate<TriangularIndex2D>
    {
        ...
        /// </summary>
        private static readonly TriangularIndex2D[] neighborOffsets = new TriangularIndex2D[]
        {
        new TriangularIndex2D(1, 0),  // Right
        new TriangularIndex2D(0, 1),  // Up
        new TriangularIndex2D(-1, 1)  // Up-Left
        };

        public int NumberOfNeighbors()
        {
            // In a triangular grid, there are typically 3 neighbors.
        ...
        {
            int dx = Mathf.Abs(x - end.value.x);
            int dy = Mathf.Abs(y - end.value.y);
